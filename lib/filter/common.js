...
async function readLines(stream) {
    return new Promise((resolve, reject) => {
        ...
        lineStream.on('line', (line) => {
            ...
            if (type === filter_1.LineType.MSG || type === filter_1.LineType.SEND) {
...
async function downloadShard(apikey, exchange, minute, channels, timeout) {
    const res = await stream_1.httpsGet(apikey, `/filter/${exchange}/${minute}`, { channels }, timeout);
