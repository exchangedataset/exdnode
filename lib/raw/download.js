...
async function downloadAllShards(clientSetting, setting) {
    ...
    for (const [exchange, channels] of entries) {
        const exchangeProms = [];
        exchangeProms.push(impl_2.snapshotDownload(clientSetting, impl_2.setupSnapshotRequestSetting({
            exchange,
            channels,
            at: setting.start,
        ...
        for (let minute = startMinute; minute <= endMinute; minute++) {
            exchangeProms.push(impl_1.filterDownload(clientSetting, impl_1.setupFilterRequestSetting({
                exchange,
                channels,
                start: setting.start,
                end: setting.end,
